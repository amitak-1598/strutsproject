function handleSingleCheckBoxClick(rowNode, rowData){
	...
	function processAll(){
		$.ajax({
			...
			error : function(status) {
				alert("Error processing refund transaction");
			}
		})		
	}
	...
	function getProcessAllPostData(){
		...
		for(index in checkedRows){
			...
			var txnType = rowElement.txnType;
			var origTxnId = '';
			if(txnType=='CAPTURE'){
				origTxnId = rowElement.origTransactionId;
			}else{
			...
			var refundObj = {
					...
					currencyCode : decodeVal(rowElement.currencyCode),
					mopType : rowElement.mopType,
					custEmail : decodeVal(rowElement.customerEmail),
			};
			refundArray.push(refundObj);
		}
		return '{' + '"refundList"'+':' + JSON.stringify(refundArray) +',' + '"token":"'+ token +'"}';
	}
