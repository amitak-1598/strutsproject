function handleSingleCheckBoxClick(rowNode, rowData){
	...
	function getProcessAllPostData(){
		...
		for(index in checkedRows){
			...
			var remittanceObj = {
					utr : rowElement.utr,
					appId : rowElement.appId,
					merchant : rowElement.merchant,
					datefrom : rowElement.dateFrom,
					netAmount : rowElement.netAmount,
					remittedDate : rowElement.remittanceDate,
					remittedAmount : rowElement.remittedAmount,
					status : rowElement.status,
					accHolderName : decodeVal(rowElement.accHolderName),
					accountNo : decodeVal(rowElement.accountNo),
			};
			remittanceArray.push(remittanceObj);
		}
		return '{' + '"remittanceList"'+':' + JSON.stringify(remittanceArray) +',' + '"token":"'+ token +'"}';
	}
