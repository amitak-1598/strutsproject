	...
	function getProcessAllPostData(){
		...
		for(index in checkedRows){
			...
			var settlementObj = {
					...
					paymentMethod: decodeVal(rowElement.paymentMethod),
					mop: decodeVal(rowElement.mopType),
					currencyCode: rowElement.currencyCode,
					txnAmount: decodeVal(rowElement.approvedAmount),
					refund: decodeVal(rowElement.refundedAmount),
					tdr: decodeVal(rowElement.tdr),
					serviceTax: decodeVal(rowElement.serviceTax),
					netAmount: decodeVal(rowElement.netAmount),
					txnDate:decodeVal(rowElement.txnDate),
