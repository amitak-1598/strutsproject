...
import com.project.strutsamit.crm.common.action.AbstractSecureAction;
import com.project.strutsamit.crm.common.action.LoginAuthenticator;
...
public class LoginAction extends AbstractSecureAction implements ServletRequestAware {
	...
	public String execute() {
		...
		try {
			...
			responseObject = loginAuthenticator.authenticate(getEmailId(), getPassword(),
					request.getHeader(CrmFieldConstants.USER_AGENT.getValue()), ipAddress);
	...
	public void validate() {
		...
		if (!(validator.isValidEmailId(getEmailId()))) {
			addFieldError(CrmFieldType.EMAILID.getName(), ErrorType.INVALID_INPUT.getResponseMessage());
			return;
		}
		 ...
		 * (!getCaptcha().equals(getCaptchaCode())){
		 * addFieldError(CrmFieldType.CAPTCHA.getName(),
